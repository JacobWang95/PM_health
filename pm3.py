...
def get_model(input_ph, gt_ph):
    ...
    loss_main = 200 * loss_rc + 1 * weight_sum - 1 * loss_pm_sum#/num_mid
...
with tf.Graph().as_default():
    ...
    for epoch in range(MAX_EPOCH):
        ...
        print mat[:10]
        print in_gt[:10]
        print 'Residual is', np.mean(np.abs(re_test - input_test[ind_test:ind_test+batch_size, ...]))
        print 'Max residual is', np.max(re_test - input_test[ind_test:ind_test+batch_size, ...])
        print 'Per of Positive', float(np.sum(input_test[ind_test:ind_test+batch_size, ...])) / (batch_size * num_feat_c)
        print 'Per of wrong RC', float(len(np.argwhere(np.abs(re_test - input_test[ind_test:ind_test+batch_size, ...]) >= 0.5))) / (batch_size * num_feat_c)
